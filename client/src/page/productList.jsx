...
{   function renderStars(rating) {
    ...
    async function fetchData(){
        try {
          await axios.get("http://localhost:3000/product/all", {
            headers: {
        ...
        <div className="p-20">
            ...
            <div><p className="font-bold text-lg text-black mr-10 ml-10 mt-10">Gợi ý hôm nay</p>
                <div className="grid grid-cols-4 gap-8 p-10 mr-10 ml-10">
                    ...
                    <Link key={index} to={`/products/${product.MaSP}`} className="max-w-sm rounded overflow-hidden shadow-lg">
                        <img
                            className="w-full"
                            src={img}
                            alt={product.TenSP}
                        ...
                        <div className="px-6 py-4">
                            <div className="mb-2">{product.TenSP}</div>
                            <div className="flex flex-row mt-4 mb-4">
                            {renderStars(product.SoSaoTrungBinh)}
